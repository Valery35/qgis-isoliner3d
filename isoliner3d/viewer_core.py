...
def bed_pairs(n_band, zband):
    """Какие пласты показывать, если выбран такой канал высот.

    Канал у грида пластов не «высота», а место в стопке: нечётный -
    кровля пласта, чётный - его подошва. Отсюда и правило, за которое
    и просили: выбрана КРОВЛЯ - показываются этот пласт и все, что
    ниже; выбрана ПОДОШВА - этот пласт и всё, что выше. Так одним
    списком выбирается и «что под этой границей», и «что над ней»,
    без второго поля.

    Возвращает список номеров каналов кровли.
    """
    n = max(int(n_band or 0), 0)
    if n < 2:
        return []
    b = max(int(zband or 1), 1)
    if b % 2:                       # кровля: этот пласт и ниже
        return list(range(b, n, 2))
    return list(range(1, b, 2))     # подошва: этот пласт и выше
