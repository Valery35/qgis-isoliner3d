...
def auto_sectors(per):
    """Сколько секторов поиска брать при таком опробовании.

    Сектора нужны скважинам: без них все ближайшие точки узла
    набираются из одного ствола, потому что проба в стволе в сотни
    раз ближе соседней скважины, и обратные расстояния вырождаются
    в ближайшего соседа.

    У плановых данных стволов нет и делить нечего, а граница сектора
    проходит лучом от узла: на ней набор соседей меняется скачком,
    и в поле появляются звёзды. Поэтому при одном-двух замерах
    в точке плана деление выключается.

    `per` это число замеров в одной точке плана. Без замера сети
    берём деление: лишним оно бывает реже, чем нужным.
    """
    if per is None:
        return 8
    return 1 if int(per) <= 2 else 8
