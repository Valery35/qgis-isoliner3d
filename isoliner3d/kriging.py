...
def weights(points, grid, vmodel, radius=None, max_points=16, sectors=8,
    ...
    out_w = np.full((n, kmax * max(int(sectors), 1)), np.nan)
    out_mu = np.full(n, np.nan)
    out_idx = np.full((n, kmax * max(int(sectors), 1)), -1, dtype=np.int64)

    for a in range(0, n, int(block)):
        b = min(a + int(block), n)
        blk = nodes[a:b]
        d2 = ((blk[:, None, :] - pts[None, :, :]) ** 2).sum(axis=2)
        d2 = np.where(d2 <= r2, d2, np.inf)
        take = _sector_take(blk, pts, d2, kmax, int(sectors))
        k = take.shape[1]
        if out_idx.shape[1] != k:
