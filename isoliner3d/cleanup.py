...
def mesh_volume(verts, faces):
    ...
    if not len(f):
        return 0.0
    # Вырожденные грани выбрасываются ДО согласования обхода. Своё
    # ребро такая грань проходит в обе стороны, и соседа по нему
    # согласование переворачивает зря, а за ним и всю его часть
    # оболочки. На кубе одна такая грань давала 833 вместо 1000,
    # на телах толщ в 2.15 объём выходил на четверть меньше.
    f = orient_faces(v, drop_degenerate(v, f))
    if not len(f):
        return 0.0
