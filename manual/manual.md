...
Прямоугольник площадки задаётся охватом либо координатами левого нижнего
угла, шириной и высотой.

## 2.02 Интерполяция точек в объёме

Ближний сосед и обратные расстояния. **Анизотропия** это отношение
вертикального масштаба к горизонтальному: при разбуривании скважинами
по вертикали данных на порядок больше, и без анизотропии интерполяция
тянула бы значения по вертикали сильнее, чем следует.

Узлы, где точек в радиусе меньше заданного минимума, остаются пропуском.

Расстояния считаются блоками узлов, поэтому время растёт с числом проб,
а не с их квадратом. Тысяча проб на куб сорок на сорок на сорок шесть
считается около секунды.
