...
# Отсечка поверхностями

Кровля и подошва меняются по площади, и плоской отметкой их не
заменить. Отсечка двумя поверхностями оставляет то, что между ними:
так убирают всё выше дневного рельефа или всё вне пласта.

В сцене это две строки свойств, любой растр проекта. Куб отсекается при чтении, до построения: оболочка, воксели и объём по блочной модели считаются по одному телу, а не расходятся между собой. В инструменте
2.03 то же самое для блочной модели: число убранных точек пишется
в журнал.

Отсекать можно и одной поверхностью. Точка, под которой поверхности
нет, не остаётся: пропустить её значит показать данные там, где
отсечка не работала.
...
Опробование идёт интервалами. Поля слоя:

| поле | что это |
|---|---|
| `hole` | номер скважины |
| `from_m`, `to_m` | интервал пробы от устья вниз, метры |
| `grade` | содержание с шумом опробования |
| `truth` | содержание по модели без шума |
| `zone` | единица внутри тела, ноль снаружи |

Шум логнормальный, отрицательных содержаний не возникает.

Для интерполяции берите `grade`. Поле `truth` нужно, чтобы отделить
ошибку метода от шума опробования: считаете куб по `grade`, а сравниваете
с `truth`. Поле `hole` нужно в 2.05, чтобы исключать скважину целиком.

Тот же список печатается в журнал 2.01 по окончании: расшифровка нужна
там, где данные только что созданы.
...
Кнопка на плашке рисует рёбра охвата, деления с подписями и стрелку
севера. Сцена без делений не даёт размера: тело выглядит одинаково
и на сто метров, и на двадцать километров. По вертикали подписываются
отметки, что для разреза важнее всего.

Деления идут по круглым числам: шаг берётся единицей, двойкой или
пятёркой на степень десяти. Края охвата не подписываются, там число
обычно некруглое.

Сетку задают свойства сцены: на каких плоскостях рисовать и с каким
шагом. Пол даёт масштаб в плане, стены по отметкам. Ноль в шаге берёт
круглый шаг от размаха, а слишком мелкий укрупняется сам.

## Стенка по линии

Третий способ показать куб это режим **Стенка по линии**. Оболочка
показывает границу тела, воксели занятые ячейки, а стенка показывает
само поле значений внутри, там где её провели.

Линия берётся из списка обрезки: нарисуйте её кнопкой рисования либо
выберите слой линий там же. Отдельного способа рисовать линию для
стенки нет, и он не нужен.

Шаг узлов вдоль линии задаётся строкой **Шаг стенки**. Ноль берёт шаг
грида: мельче него данных всё равно нет. По вертикали берутся уровни
куба.

Значения выбираются трилинейно, поэтому стенка выходит гладкой,
а не ступенчатой. За краями куба возвращается пропуск, а не край:
продлить край наружу значило бы показать данные там, где их нет.
Треугольник, у которого хоть один узел без данных, не строится вовсе.

Ломаная с изломом обходится общим шагом вдоль всей длины, а не
по отрезкам: на изломе стенка не рвётся.

Стенка дешёвая. На кубе сто на сто на шестьдесят ломаная из трёх узлов
при шаге десять метров даёт семь тысяч узлов и четырнадцать тысяч
треугольников, треть мегабайта.

## Разрез тела

Тело режется контуром, коридором вдоль линии и диапазоном отметок,
и срез закрывается крышкой: видно сечение, а не изнанку.

Для этого тело должно быть замкнутым. Собирайте его в 2.04 **со снятым
слиянием соседних граней**: слияние делает слой в разы легче, но рвёт
границу Т-образными стыками, и крышку такому телу не построить.
Слой выйдет вдвое тяжелее — это цена замкнутости.

Отвесные грани режутся вдоль отрезка, в который они вырождаются
в плане: как полигон их не разрезать, площадь нулевая. Без этого стенка
торчала бы за контур, а соседняя горизонтальная была бы обрезана
по нему, и между ними осталась бы щель.

В журнал идут числа, по которым видно, что произошло: сколько граней
осталось, расстояние от линии до данных, сколько тел не замкнуто
до резки, сколько краевых рёбер легло на контур среза и сколько
полигонов крышки собралось.

## Оболочки таблицей

Оболочки задаются таблицей: строка на уровень, столбцы уровень, цвет
и непрозрачность, плюс галка показа. Два списка, связанные по порядку,
съезжают молча: пропустил цвет, и все остальные встали не на свои
уровни. В строке съехать нечему.

Пустая ячейка берёт автоматическое: цвет по номеру оболочки,
непрозрачность растёт наружу. Пустая таблица это отсечка и одна
оболочка. Цвет выбирается двойным щелчком, строка удаляется правой
кнопкой, а новая появляется сама, когда заполнена последняя.

**Закрывать выход на край куба** ставит крышку там, где тело упирается
в границу куба. Без неё оболочка незамкнута: тело выглядит вскрытым,
и объём по нему не посчитать. По умолчанию выключено.

Внутренние оболочки рисуются раньше наружных: нарисованная первой
закрывает собой то, что за ней, и наружная, идя первой, съедала бы все
внутренние.

## Оболочки в слой проекта

Кнопка на плашке кладёт оболочки выбранного слоя в слой проекта.
Берётся то, что настроено и видно: уровни, крышка на краю, сглаживание
и отброс мелочи. Набирать их заново не надо.

Оболочка сразу разбирается на **связные тела**, и объём каждого
считается тут же. Разбиение составной геометрии средствами QGIS для
этого не годится: оно даёт отдельные треугольники, связность там
не считается вовсе.

Тело на объект, в полях: номер, уровень, цвет, число граней,
замкнутость, объём, диапазон отметок. Плюс два поля о качестве
оболочки. **holes** это рваные рёбра: у дыры объём бессмыслен.
**pinch** это касания тела самого себя: дыр нет, объём точен.
Мелкие дыры зашиваются сами, большие остаются как есть - затянув
пол-оболочки плоской заплатой, получишь объём, который выглядит
настоящим и неверен.
...
## Чистка изоповерхности

Маршевая поверхность идёт ступенями по ячейкам куба, и мелкие обрывки
на ней шумят. Две строки в свойствах слоя: **сглаживание** и
**отбросить куски мельче**.

Мелочь отбрасывается до сглаживания: иначе обрывок успевает затянуть
к себе соседей, и после удаления рядом остаётся вмятина. Края
не двигаются, потому что крышка на срезе строится по краевым рёбрам.

Если порог убирает всё, поверхность остаётся как была. Сглаживание
слегка ужимает тело, поэтому для подсчёта объёма берите несглаженное.

## Выгрузка для CAD

Кнопка на плашке пишет сцену в STL или OBJ, формат выбирается
расширением. GLB несёт цвет и прозрачность и годится для просмотра,
а в CAD нужна замкнутая оболочка, из которой делают тело.

STL двоичный, частей в формате нет: несколько оболочек сливаются
в одну. OBJ текстовый, части остаются группами со своими именами,
зато файл больше.

Короб, сетка и подписи в CAD не идут: там нужны тела, а не украшение
вида. Отметки настоящие, про преувеличение не спрашивается.

В итоге говорится, сколько тел вышло замкнутыми: незамкнутое CAD
покажет мешем, но телом не сделает.
...
метров по мощности иначе сплющивается в блин. Преувеличение считается
от общей середины всех тел, иначе растянулся бы рельеф внутри каждого,
а расстояния между ними остались прежними.

**Координатный короб уходит в файл всегда**, показан он на экране или
нет: это разные решения. На экране короб мешает смотреть, а в файле
без него нет масштаба.

Рёбра, штрихи и сетка пишутся линиями, подписи делений — полосками.
Толщину линий в glTF задать нельзя, её выбирает просмотрщик, и обычно
это один пиксель, который на большой модели теряется. Полоска даёт
настоящую толщину, в метрах.

Подписи рисуются чертёжным шрифтом: цифра из отрезков. Текста в glTF
не бывает — есть либо геометрия букв, либо картинка на плоскости,
а плоская картинка при повороте встаёт ребром и пропадает.

Итог выгрузки виден в окне: сколько тел, есть ли короб, применено ли
преувеличение, размер файла. Если вместо него написано «Выгрузка
не удалась», там же будет причина.
...
| Куб вышел ступенями по номерам | В поле значения взято `hole`. | Взять `grade`: 2.02 подставляет первое числовое поле, а это номер скважины. |
| Звёзды и лучи в поле интерполяции | Деление на сектора у плановых проб. | Оставить ноль в поле секторов: он берётся от данных. У проб в плане деления не будет. |
...
| Стенка не строится, просит линию | Линия не нарисована и слой не выбран. | Нарисовать линию кнопкой рисования либо выбрать слой линий в списке обрезки. |
| Стенка вышла пустой | Линия за пределами куба. | Проверить охват куба: за его краями значений нет, и стенка там не строится. |
| В теле дырки после разреза | Тело собрано со слиянием граней и не замкнуто. | Пересобрать в 2.04 со снятым слиянием: в журнале видно число незамкнутых тел. |
| В выгрузке нет короба или подписей | Стоит сборка до 0.65.7. | Номер сборки виден в заголовке окна; итог выгрузки печатается в самом окне. |
