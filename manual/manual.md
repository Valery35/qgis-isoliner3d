...
Слева панель управления, справа сцена. Сцена вращается мышью, зум колесом.
Крупные гриды прореживаются автоматически, чтобы в сцене было не больше
60 тысяч узлов на слой: показ остаётся быстрым на матрицах любого размера,
а форма поверхности сохраняется.

Панель разбита на три вкладки. **Слои** - растры проекта и персональные
параметры каждого. **Векторы** - скважины и плоскость разреза.
**Тела** - полигональные слои с Z.

Под вкладками живут общие настройки сцены: вертикальное преувеличение,
разнос по Z, прозрачность поверхностей, кнопки видов **Сверху** и
**Сбоку**, **Снимок PNG…** и **Обновить сцену**. Настройки под вкладками
действуют сразу на всю сцену, параметры внутри вкладки **Слои** - только
на выделенный слой.
...
# Виды, снимок и прочее

**Сверху** и **Сбоку** ставят ортогональные виды, от которых удобно
отталкиваться при подборе ракурса. **Снимок PNG…** сохраняет кадр сцены
в файл для отчёта или презентации, размер снимка равен размеру окна сцены,
поэтому перед съёмкой окно имеет смысл развернуть.
